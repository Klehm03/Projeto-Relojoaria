...
def criar_tela_login():
    ...
    def abrir_nova_janela():
        ...
        # Se já houver uma janela anterior, feche-a
        if janela_anterior is not None:
            janela_anterior.destroy()
        
        # Cria uma nova janela
        janela_anterior = tk.Tk()
        janela_anterior.title("Nova Janela")
        
        # Adiciona um botão para abrir outra nova janela
        btn_nova_janela = tk.Button(janela_anterior, text="Abrir outra janela", command=abrir_nova_janela)
        btn_nova_janela.pack(pady=20)
        
        # Inicia o loop da interface gráfica
 

    # Chama a função para abrir a primeira janela
